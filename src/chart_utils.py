...
def annotate_bars(ax, fmt="{:.1f}", prefix="", suffix="", fontsize=9):
    for container in ax.containers:
        labels = []
        for bar in container:
            value = bar.get_height()
            labels.append(f"{prefix}{fmt.format(value)}{suffix}")
        ax.bar_label(container, labels=labels, padding=3, fontsize=fontsize, color="#333333")


def annotate_barh(ax, fmt="{:.2f}", prefix="", suffix="", fontsize=9):
    for container in ax.containers:
        labels = []
        for bar in container:
            value = bar.get_width()
